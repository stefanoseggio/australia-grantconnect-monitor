...
3. Memory is written **only for records that were actually stored** - and records are delivered oldest-first - so a spending limit, a timeout or a platform migration half-way never loses an award: the next run simply picks it up. If a run hits `maxItems` before the walk is complete, the Actor also remembers how deep it got and the next run continues below the block it already delivered instead of stopping at it. The memory holds about a year of activity (50,000 entries).
